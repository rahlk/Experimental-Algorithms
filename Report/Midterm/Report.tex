...
\begin{document}
	...
	HPC supports upto 16 processors with 128GB of shared memory. The runtimes for DE and GALE when deployed on 2 to 16 processors is illustrated in figure \ref{fig:de_runtime} and figure \ref{fig:gale_runtime} respectively. The results show that after 8 processors the runtime for DE starts to become constant and for GALE, the results does not stabilize. This might be because the runtimes for GALE is two orders of magnitude more than that of DE and hence the communication overhead might not be significant in its case.
	
	\begin{figure}[h]
		\begin{mdframed}
			\includegraphics[width=\linewidth]{img/de_processors}
		\end{mdframed}
		\caption{DE: Runtime vs Processors}	
		\label{fig:de_runtime}
	\end{figure}
